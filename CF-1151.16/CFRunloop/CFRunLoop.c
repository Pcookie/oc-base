...
//端口分配
#pragma mark -- CFPortAllocate 完成了wakeupport创建
...
#endif
#pragma mark -- __CFSendTrivialMachMessage
static uint32_t __CFSendTrivialMachMessage(mach_port_t port, uint32_t msg_id, CFOptionFlags options, uint32_t timeout) {
    ...
    // 从上个函数传过来的是wakeupport  主要是这句 这句发送了一个消息给RL - > _ wakeUpPort唤醒了runloop
...
struct __CFRunLoopMode {
    CFRuntimeBase _base;
    pthread_mutex_t _lock;	/* 必须在锁定之前给循环池加锁 must have the run loop locked before locking this */
    CFStringRef _name;
    Boolean _stopped;
    char _padding[3];
...
/* Bit 0 of the base reserved bits is used for stopped state 保留位0用于标示状态 stop */
/* Bit 1 of the base reserved bits is used for sleeping state 睡眠 */
/* Bit 2 of the base reserved bits is used for deallocating state 释放销毁 */
...
struct __CFRunLoopSource {
    CFRuntimeBase _base; // 一个最基本的对象 cfisa info[4]
    uint32_t _bits; // 位
    pthread_mutex_t _lock; // 锁
    CFIndex _order;	 //不可改变的 命令、规则	/* immutable */
...
#pragma mark -- __CFRunLoopServiceMachPort wakeupport 收到消息后由这个方法处理
static Boolean __CFRunLoopServiceMachPort(mach_port_name_t port, mach_msg_header_t **buffer, size_t buffer_size, mach_port_t *livePort, mach_msg_timeout_t timeout, voucher_mach_msg_state_t *voucherState, voucher_t *voucherCopy) {
    ...
    for (;;) {		/* In that sleep of death what nightmares may come ... */
        ...
        // 此方法执行后会驻留在这句  等待消息 等ret = __CFSendTrivialMachMessage(rl->_wakeUpPort, 0, MACH_SEND_TIMEOUT, 0);这句执行完后 继续执行下面这句  形成了闭环
...
#endif
#pragma mark --- 这里执行了CFRunLoopTimeout
...
//    当上面这句代码执行后 ret = mach_msg...将收到消息继续执行。闭环
