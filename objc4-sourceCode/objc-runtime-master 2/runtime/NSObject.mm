...
#   define POOL_BOUNDARY nil //边界 哨兵对象
...
#else
        PAGE_MAX_SIZE;  // size and alignment, power of 2 // 4096即虚拟内存一页的大小
#endif
    static size_t const COUNT = SIZE / sizeof(id);

    magic_t const magic; // 用来校验autoreleasePoolPage的结构是否完整
    id *next; // next指向最新添加的autoreleased对象的下一个位置，初始化时指向begin()
    pthread_t const thread; // 指向当前线程
    AutoreleasePoolPage * const parent; // 指向父结点，第一个结点的parent值为nil
    AutoreleasePoolPage *child; // 指向子结点，最后一个结点的child值为nil
    ...
    static inline id *autoreleaseFast(id obj)
    {
        ...
        if (page && !page->full()) { // 当前page存在且没有满时，直接将对象添加到当前page中，即next指向的位置
            return page->add(obj);
        } else if (page) { // 当page存在且已满时，创建一个新的page，并将对象添加到新创建的page中
            return autoreleaseFullPage(obj, page);
        } else { // 当前page不存在时，即还没有page时，创建第一个page，并将对象添加到新创建的page中
...
public:
    static inline id autorelease(id obj)
    {
        ...
        //和push相似，不过push插入的是POOL_BOUNDARY 而这函数插入一个autoreleased对象
        return obj;
    }
...
{
    // ctxt 就是 push函数的返回值，也就是POOL_BOUNDARY的内存地址，即pool token 。当执行pop操作时，内存地址在pool token之后的所有autoreleased对象都会被release。直到pool token所在page的next指向pool token为止
